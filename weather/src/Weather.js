...
function Weather() {
    ...
    useEffect(()=>{
        ...
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${areaValue}&units=metric&appid=1cf20e60cde475059fbdbf064223ba4f`,init)
        .then( async response =>{
            try{
...
/* 첫화면에 나올 정보 */
                    return(
                        `
                            ...
                            <div>
                                ...
                                최고기온 : ${data.main.temp_max}&deg; <br>
                                최저기온 : ${data.main.temp_min}&deg; &nbsp;&nbsp;&nbsp;
                                체감온도 : ${data.main.feels_like}&deg; <br>
                            </div>
                            <div>
    ...
    useEffect(()=>{
        ...
        }).join(''); // 배열을 하나의 문자열로 합칠때 사용하는 메서드
        areaList.innerHTML = areaWrite;
    },[]);

    /* select 값 변경 시 날씨정보 해당 도시로 바뀌도록 설정 */
    ...
    return (
        <section>
            ...
            <select id='select' className='areaList' onChange={changeSelect}></select>
        </section>
    )
