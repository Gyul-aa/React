...
function Header() {
    /* 메뉴 폴더 아이콘 */
    const gnbFolder = { background: 'url("./svg/folder.svg") no-repeat', backgroundSize:'cover'}
    const gnbFile = { background: 'url("./svg/folder__file.svg") no-repeat', backgroundSize:'cover'}
    ...
    useEffect(()=>{
      ...
      setInterval(function(){
        today = new Date();
        hh = (today.getHours()<10?'0'+today.getHours():today.getHours()); // 1시~9시 앞에 0 붙이기
        mm = (today.getMinutes()<10?'0'+today.getMinutes():today.getMinutes()) // 1분~9분 앞에 0 붙이기
        ...
        <>
            ...
            <header>
                ...
                <ul className="gnb">
                    ...
                    <li><Link to="/synopsys">Synopsis</Link></li>
